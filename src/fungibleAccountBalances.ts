import { GatewayApiClient, ResourceAggregationLevel } from "@radixdlt/babylon-gateway-api-sdk";
import { FungibleResourceBalance } from "./types";
import { extractStringArrayMetadata, extractStringMetadata, extractUrlMetadata } from "./data_extractors/metadata";
...
export async function getFungibleBalancesForAccount(gatewayApi: GatewayApiClient, address: string): Promise<FungibleResourceBalance[]> {
    ...
    return balances.items.flatMap((item) => {
        if (item.aggregation_level !== ResourceAggregationLevel.Global) throw new Error("Unexpected aggregation level");
        ...
        const tokenIconUrl = extractUrlMetadata(tokenInfoItem.metadata, "icon_url");
        const tokenInfoUrl = extractUrlMetadata(tokenInfoItem.metadata, "info_url");
        const tokenTags = extractStringArrayMetadata(tokenInfoItem.metadata, "tags");
