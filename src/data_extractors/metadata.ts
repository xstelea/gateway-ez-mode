...
export function extractStringMetadata(metadataItems: EntityMetadataCollection, key: string): string | null {
...
export function extractUrlMetadata(metadataItems: EntityMetadataCollection, key: string): string | null {
    const item = metadataItems.items.find((item) => item.key == key);
    if (!item) return null;
...
export function extractStringArrayMetadata(metadataItems: EntityMetadataCollection, key: string): string[] | null {
    const item = metadataItems.items.find((item) => item.key == key);
    if (!item) return null;
