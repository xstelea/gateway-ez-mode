import { NetworkId, NotarizedTransaction, PrivateKey, RadixEngineToolkit, TransactionBuilder, TransactionHeader, TransactionManifest, generateRandomNonce, InstructionsKind, Convert } from "@radixdlt/radix-engine-toolkit";
...
describe('transactionPoller', () => {
    it('should successfully be able to poll a stokenet transaction', async () => {
        const gateway = new GatewayEzMode(GatewayApiClient.initialize({
            applicationName: '',
            networkId: NetworkId.Stokenet
        }))

        // Create a minimal transaction that only locks some fee with just a random signer
        const [transaction, intentHash] = await createMinimalStokenetTransaction()
